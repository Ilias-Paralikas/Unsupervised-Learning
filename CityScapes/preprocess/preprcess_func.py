...
def progan_preprocess_cityscapes(root='./CityScapes/data',
...
def msg_preprocess_cityscapes(root='./CityScapes/data',
                          output_root='./preprocessed_data',
                          sizes=[512],
                          preprocess_labels=True,
                          output_activation="tanh"):
    ...
    # Sort sizes based on area (h * w) to ensure index 0 is the smallest
    processed_sizes.sort(key=lambda x: x[0] * x[1])
    ...
    output_images_base = os.path.join(output_root, output_activation, "multi_res", "images")
    output_targets_base = os.path.join(output_root, output_activation, "multi_res", "labels")

    print(f"Processing {len(image_paths)} images into multi-res list format...")

    # ---- Main loop ----
    for img_path in tqdm(image_paths, desc="Processing"):
        image = Image.open(img_path).convert("RGB")
        
        # 1. Process all sizes for the image
        img_list = [image_transforms[size](image) for size in processed_sizes]
        
        # Save images
        rel_path = os.path.relpath(img_path, images_dir)
        save_path_img = os.path.join(output_images_base, rel_path).replace(".png", ".pt")
        os.makedirs(os.path.dirname(save_path_img), exist_ok=True)
        torch.save(img_list, save_path_img)

        # 2. Process all sizes for the label
        if preprocess_labels:
            target_path = (img_path.replace("leftImg8bit_trainextra/leftImg8bit", "gtCoarse")
                                   .replace("_leftImg8bit.png", "_gtCoarse_labelIds.png"))
            
            if os.path.exists(target_path):
                target = Image.open(target_path)
                lbl_list = [label_transforms[size](target) for size in processed_sizes]
                
                rel_target_path = os.path.relpath(target_path, targets_dir)
                save_path_lbl = os.path.join(output_targets_base, rel_target_path).replace(".png", ".pt")
                os.makedirs(os.path.dirname(save_path_lbl), exist_ok=True)
                torch.save(lbl_list, save_path_lbl)
